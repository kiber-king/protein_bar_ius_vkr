...
from .serializers import (
    ...
    ProductionSettingsSerializer, NotificationSerializer
)
...
class BatchViewSet(viewsets.ModelViewSet):
    ...
    def simulate_parameter(self, request, pk=None):
        ...
        if not last_parameter:
            ...
            temperature = last_parameter.temperature + random.uniform(-5.0, 5.0)
            pressure = last_parameter.pressure + random.uniform(-0.3, 0.3)
            mixing_speed = last_parameter.mixing_speed + random.uniform(-2.0, 2.0)
            glazing_thickness = last_parameter.glazing_thickness + random.uniform(-0.2, 0.2)
        
        # Проверяем критические значения для выявления брака
        is_defect = (
            temperature < 160.0 or temperature > 180.0 or
            pressure < 2.0 or pressure > 3.0 or
            mixing_speed < 55.0 or mixing_speed > 65.0 or
            glazing_thickness < 1.8 or glazing_thickness > 2.2
        )
